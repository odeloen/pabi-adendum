...
use App\Ods\Elearning\Course\Infrastructure\Persistence\Eloquent\Models\PublishedTopicDataModel;
use Carbon\Carbon;
use Ramsey\Uuid\Uuid;

class PublishedTopicRepository implements ITopicRepository
{
    /**
    ...
    private function mapDataModelToDomainModel(PublishedTopicDataModel $topicDataModel){
    ...
    private function mapDomainModelToDataModel(Topic $topicDomainModel, PublishedTopicDataModel $topicDataModel){
    ...
    {
        $topicDataModels = PublishedTopicDataModel::where('course_id', $courseID)->orderBy('created_at', 'asc')->get();
    ...
    {
        $topicDataModel = PublishedTopicDataModel::find($topicID);
    ...
    {
        $topicDataModel = new PublishedTopicDataModel();
    ...
    {
        $topicDataModel = PublishedTopicDataModel::find($topicDomainModel->getId());
        $topicDataModel = $this->mapDomainModelToDataModel($topicDomainModel, $topicDataModel);
        $topicDataModel->updated_at = Carbon::now()->toDateTimeString();
