...
<?php
/**
 * @var \App\Ods\Elearning\Course\Presenter\Models\CourseDetailViewModel $data
 */
?>
@section('addcss')
...
@if (!empty($data->course))
    @if ($data->course->isLocked())
        <div class="alert alert-warning alert-styled-left">
            Kelas ini sedang dalam proses verifikasi admin.
        </div>
    @endif
    @if ($data->course->isLocked() && $data->course->isDeleted())
...
@if (!empty($data->topics))
    @foreach ($data->topics as $topic)
        <div class="panel">
            ...
            @endif
                <h6 class="panel-title">{{$topic->name}}<a class="heading-elements-toggle"><i class="icon-more"></i></a></h6>
                @if ($topic->isModified())
                    {!!$topic->getActionTag()!!}
                @endif
                <div class="heading-elements">
                    <ul class="icons-list">
                        <li class="dropdown" style="margin-right:5px;">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li><a onclick="onClickUpdateTopic('{{$topic->id}}')" data-toggle="modal" data-target="#modal_topicEdit">Edit Topik</a></li>
                                <li>
                                    <form action="{{route('elearning.lecturer.topic.delete')}}" method="post">
                                        @csrf
                                        <input name="topic_id" value="{{$topic->id}}" type="hidden">
                                        <button type="submit" class="astext hapus-topik" value="{{$topic->id}}">Hapus Topik</button>
                                    </form>
                                </li>
                            </ul>
                        </li>
            ...
            <div class="panel-body">
                ...
                @if(!empty($topic->materials) && $topic->materials->count() > 0)
                    <div class="text-right mt-5">
                @else
                    <div class="text-center mt-5">
                @endif
                    <a href="{{route('elearning.lecturer.material.create', [$data->course->id, $topic->id])}}">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover" style="font-size:13px;">
                            ...
                            @if(!empty($topic->materials))
                                ...
                                <tr class="clickable-row" href="{{route('elearning.lecturer.material.show', [$data->course->id, $topic->id, $material->id])}}"style="cursor:pointer;">
                                    <td class="clickable" style="width:1%;white-space:nowrap;"><i class="{{$material->getIcon()}}"></i></td>
                                    <td class="clickable">{{$material->name}}</td>
                                    <td class="clickable" style="width:1%;white-space:nowrap;">
                                        <center>
                                            @if ($material->isModified())
                                                {!!$material->getActionTag()!!}
                                            @else
                                                {{$material->updated_at_string}}
                                            @endif
                                        </center>
                                    </td>
                                    <td style="width:1%;white-space:nowrap;" >
                                        <ul class="icons-list">
                                            <li class="dropdown">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-right">
                                                    <li><a href="{{route('elearning.lecturer.material.edit', [$data->course->id, $topic->id, $material->id])}}">Edit Materi</a></li>
                                                    <li>
                                                        <form action="{{route('elearning.lecturer.material.delete')}}" method="post">
                                                            @csrf
...
<div id="modal_topicAdd" class="modal fade" tabindex="-1" style="display: none;">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{route('elearning.lecturer.topic.create')}}" method="POST">
                @csrf
                <input type="hidden" name="course_id" value="{{$data->course->id}}">
...
<div id="modal_topicEdit" class="modal fade" tabindex="-1" style="display: none;">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{route('elearning.lecturer.topic.update')}}" method="POST">
                @csrf
...
<div id="modal_classEdit" class="modal fade" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{route('elearning.lecturer.course.update')}}" method="POST" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="course_id" value="{{$data->course->id}}">
                <div class="modal-body">
                    <div class="form-group">
                        <div class="row">
                            <div class="col-sm-12">
                                <h6>Nama Kelas</h6>
                                <input name="name" type="text" class="form-control" value="{{$data->course->name}}">
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="row">
                            <div class="col-sm-12">
                                ...
                                <figure class="text-center">
                                    <img class="gambar img-responsive img-thumbnail" id="item-img-output" src="{{env('APP_URL')}}/sl/images/{{$data->course->image_path}}" style="max-height:300px;width:auto;"/>
                                </figure>
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="row">
                            <div class="col-sm-12">
                                <h6>Deskripsi</h6>
                                <textarea name="description" class="form-control maxlength-textarea" maxLength="250" rows="2" style="resize:vertical">{{$data->course->description}}</textarea>
                            </div>
                        </div>
                    </div>
                    {{-- <div class="form-group">
...
<div class="sidebar sidebar-opposite sidebar-default sidebar-separate">
    <div class="sidebar-content">
        ...
        <a href="{{route('lecturer.submission.list', $data->course->id)}}">
        ...
        <div class="thumbnail">
            <div class="thumb">
                @if (!empty($data->course->image_path != null))
                    <img src="{{env('APP_URL')}}/sl/images/{{$data->course->image_path}}" class="img-responsive img-rounded" alt="">
                @else
            ...
            <div class="caption">
                <div class="content-group-sm media">
                    <div class="media-body">
                        <h6 class="text-semibold no-margin">{{$data->course->name}}</h6>
                        <ul class="list-inline list-inline-separate no-margin text-muted">
                            <li>oleh <span class="text-primary">{{$data->course->lecturer}}</span></li>
                            <li>
                                {{-- @if (!empty($data->course->categories))
                                    @foreach ($data->course->categories as $category)
...
<form id="form_course_delete" action="{{route('elearning.lecturer.course.delete')}}" method="POST">
    @csrf
    <input type="hidden" name="course_id" value="{{$data->course->id}}">
</form>
@endif
...
<script>
    ...
    $('.tutup-kelas').on('click', function(){
        ...
        swal({
            title: 'Apakah anda yakin akan menutup Kelas {{$data->course->name}}?',
    ...
    $('.hapus-topik').on('click', function(e){
        ...
        swal({
            title: 'Apakah anda yakin akan menghapus Topik '+topic.name+'?',
            text: 'Semua materi dalam Topik '+topic.name+' yang pernah anda buat akan menghilang',
            type: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
...
<script>
    ...
    function setUpdateTopicModal(topic){
        $('#update_topic_id').val(topic.id)
        $("#update_topic_name").val(topic.name)
        $("#update_topic_description").text(topic.description)
    }
